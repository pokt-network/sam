...
- **Auto top-up fund-tx sequence race** — fund txs from the bank in a single worker cycle used to all re-query the bank's on-chain sequence before any of them got included in a block (pocketd's default broadcast-mode is sync, not block), so the second and later txs failed with `account sequence mismatch, expected N, got N-1`. The worker now queries the bank's `account_number + sequence` once at the start of each per-network cycle (via `/cosmos/auth/v1beta1/accounts/{addr}`) and threads a locally-incremented sequence into each fund tx via explicit `--account-number / --sequence` flags. On the rare external-desync case (admin runs `pocketd` from a shell while a cycle is in flight) the executor parses the chain-reported expected sequence from the error and retries once. App-signed ops (stake/upstake/unstake/delegate) were never affected; only bank-signed funds raced.
