...
- **Auto top-up out-of-gas + fund loop** — pocketd's `--gas=auto` simulator does not model the unordered-tx nonce-dedup store write, so the simulated gas landed ~0.3% below actual execution (production: 50,959 simulated vs 51,137 actual). Broadcasts succeeded at mempool (`tx_hash` returned) but the on-chain execution failed with `code 11 out of gas`, so the same app got "funded" repeatedly without ever actually receiving POKT. Two fixes: (1) every tx now uses `--gas-adjustment 1.5` on top of `--gas=auto`, giving the auto-estimate plenty of headroom for the missing store write plus future chain growth; (2) the auto top-up phase-2 (upstake) trigger now fires as soon as `liquid >= amountNeeded` rather than `liquid >= amountNeeded + reserve`, so a few-uPOKT shortfall below the reserve target no longer re-fires a full fund tx — the reserve is still aimed at when funding, but it's no longer a precondition for upstake.
